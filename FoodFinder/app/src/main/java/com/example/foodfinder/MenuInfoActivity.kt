...
class MenuInfoActivity : AppCompatActivity() {
    ...
    private lateinit var imagePathString: String    // 이미지 경로
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(intent.hasExtra("recognizedText")) {      // intent로 받아온 인식된 텍스트가 있을 경우
            imagePathString = intent.getStringExtra("imagePath").toString()
            koreanText = intent.getStringExtra("recognizedText").toString()

            menuname_kr_tv.text = koreanText

            // 한국어 -> 외국어 번역
            translateText(koreanText, menuname_us_tv)
            // 한국어 -> 로마자 번역
        ...
        back_btn.setOnClickListener {
            // MenuRecognitionActivity로 이동하는 Intent 생성
            val intent = Intent(this, MenuRecognitionActivity::class.java)
            intent.putExtra("imagePath", imagePathString)
            intent.putExtra("recognizedText", koreanText)
            // Intent로 새 액티비티 시작
    ...
    private fun translateText(resultText: String, menuname_us_tv: TextView) {
